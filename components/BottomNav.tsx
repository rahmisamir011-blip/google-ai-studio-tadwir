...
export const BottomNav: React.FC<BottomNavProps> = ({ activeScreen, onNavigate, onVoiceClick }) => {
    ...
    const handleVoiceClick = () => {
        if (navigator.vibrate) {
            navigator.vibrate(80); // A stronger vibration for the main action
        }
        onVoiceClick();
    };

    return (
        <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-sm shadow-[0_-2px_10px_rgba(0,0,0,0.1)] flex items-center justify-around z-20">
            {/* Left side */}
            ...
            <div className="w-1/5 flex justify-center">
                <button 
                    ...
                    className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center -mt-8 shadow-lg transform transition-transform hover:scale-110 active:scale-100"
